...
# -*- coding: utf-8 -*-
"""
train_quality_model.py — 动作质量二分类模型训练脚本

功能:
    1. 加载全部 65 个 .mat 文件，预处理 + 事件检测 + 特征提取
    2. 从 labels_v2.csv 读取质量标签（标准/不标准）
    3. 候选模型 GridSearchCV(cv=5) 超参数搜索
    4. 选择最优模型（以 F1(macro) 为指标）
    5. 使用 class_weight='balanced' 处理类别不均衡

输出:
    emg/models/quality_model.joblib
    emg/models/scaler_quality.joblib
    emg/outputs/quality_*.csv / .png

使用方式:
    cd f:/Project_final
    D:/Anaconda_202410/python.exe train_quality_model.py
"""
...
# ------------------------------------------------------------
def load_all_data():
    """加载全部 65 个文件，提取特征，用 labels_v2.csv 匹配标签"""
    # 加载标签
    labels_df = pd.read_csv(LABELS_FILE)
    # 构建 (filename, cycle_id) -> quality_label 的快速查找
    label_map = {}
...
# 按文件分层划分
# ------------------------------------------------------------
def stratified_file_split(df, file_col="filename", label_col="quality_label",
                          test_size=0.20):
    """按文件分层划分训练/测试集"""
    unique_files = df[file_col].unique()
    file_to_label = {}
    for f in unique_files:
        labels = df[df[file_col] == f][label_col].values
    ...
    train_mask = df[file_col].isin(train_files)
    test_mask = df[file_col].isin(test_files)
...
def main():
    print("=" * 64)
    print("  动作质量二分类模型训练 (GridSearchCV, cv=5)")
    print("=" * 64)
    ...
    train_mask, test_mask = stratified_file_split(df, test_size=0.20)
    X_train, X_test = X[train_mask], X[test_mask]
    y_train, y_test = y[train_mask], y[test_mask]
    # 用于 GroupKFold 的分组：同一文件的所有周期必须在同一折
    groups_train = file_ids[train_mask]
    print(f"  X_train: {X_train.shape}, X_test: {X_test.shape}")
    print(f"  y_train 分布: {dict(zip(*np.unique(y_train, return_counts=True)))}")
    print(f"  y_test  分布: {dict(zip(*np.unique(y_test, return_counts=True)))}")
    ...
    df_results.to_csv(os.path.join(OUTPUT_DIR, "quality_model_comparison.csv"),
                      index=False, encoding="utf-8-sig")
    print(f"  对比表已保存: {OUTPUT_DIR}/quality_model_comparison.csv")
    ...
    plot_confusion_matrix(cm, le.classes_,
                          f"质量分类混淆矩阵 ({best['model']})",
                          os.path.join(OUTPUT_DIR, "quality_confusion_matrix.png"))

    final_clf = best_model.named_steps["clf"]
    plot_feature_importance(final_clf, feature_names,
                            f"特征重要性排名 ({best['model']})",
                            os.path.join(OUTPUT_DIR, "quality_feature_importance.png"))

    plot_learning_curve(best_model, X_train, y_train,
                        f"学习曲线 ({best['model']})",
                        os.path.join(OUTPUT_DIR, "quality_learning_curve.png"))

    report = classification_report(y_test, y_pred_best, target_names=le.classes_)
    print("\n分类报告:\n" + report)
    with open(os.path.join(OUTPUT_DIR, "quality_classification_report.txt"),
    ...
    dump(le, os.path.join(MODEL_DIR, "label_encoder_quality.joblib"))
    print(f"\n  模型已保存: {model_path}")
